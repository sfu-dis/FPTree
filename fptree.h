...
//#define AVX512	// not used with tbb, hinders scalability
...
// offset contains some 0s on most significant bits when bitmap size < 64
class Bitset {
...
public:
    ...
    inline size_t count() {
        size_t set_count = 0;
        for (uint64_t val = bits; val; ++set_count)
            val &= (val - 1);
        return set_count;
    }
