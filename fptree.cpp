...
bool BaseNode::isLocked()
{
    return ((this->versionLock & 0b1) == 0b1);
}
...
void BaseNode::writeUnlock()
{
    ...
    this->versionLock = this->versionLock & 0xfffffffffffffffe;
}
...
LeafNode* FPtree::findLeafAndLock(uint64_t key)
{
    if (!root)
        return 0;
restart:
    ...
    pLeafNode->upgradeToWriteLockOrRestart(currentVersion, needRestart);
    if (needRestart) { goto restart; }
...
restart:
    ...
    *ancestor = nullptr;
    ...
    if (needRestart || (first != root))
    {
        if (restartCounter++ == 10000000)
            printf("Failed to acquire root: %p in 10000000 tries!\n", first);
        goto restart;
    } 

    if ( (first == root) ||
             !((reinterpret_cast<InnerNode*> (first))->nKey < MAX_INNER_SIZE) )
    {
    ...
    if (first->isInnerNode)
    {
        ...
        while (true)
        {
            ...
            // {
            //     if (restartCounter++ == 10000000)
            //         printf("Failed checkOrRestart: %p in 10000000 tries!\n", first);
            //     first->writeUnlock();
            //     goto restart;
            // }
            ...
            {
                if (restartCounter++ == 10000000)
                    printf("Failed second->readLockOrRestart: %p in 10000000 tries!\n", first);
                first->writeUnlock();
                goto restart; 
            } 
            ...
            // {
            //     if (restartCounter++ == 10000000)
            //         printf("Failed second->upgradeToWriteLockOrRestart: %p in 10000000 tries!\n", first);
            //     first->writeUnlock();
            //     goto restart; 
            // } 
...
uint64_t FPtree::rangeScan(uint64_t key, uint64_t scan_size, char* result)
{
    ...
    while (true) 
    {
        if ((leaf = findLeafAndLock(key)) == nullptr)
            return 0;
        ...
        restart:
            needRestart = false;
            next_leaf->writeLockOrRestart(needRestart);
            if (needRestart) { goto restart; }
