...
//#define PMEM // comment this out for using DRAM as NVM
   ...
   #define MEMPOOL
#else
   #define MEMPOOL   // use default thread local mempool, comment out to use new/jemalloc
#endif
...
public:
   ...
   void init(int num_workers, long long size, long long align)
   {
      mempool_cur = (char *)memalign(align, size);
      mempool_end = mempool_cur + size;
      if (!mempool_cur)
      {
        perror("malloc");
        exit(1);
   ...
   void *alloc(unsigned long long size)
   {
      ...
      fprintf(stderr, "alloc - run out of memory!\n");
      abort();
   }
};
...
#define the_mempool (the_thread_mempools.tm_pools[0])
...
#define the_nvmpool (the_thread_nvmpools.tm_pools[0])
...
#else // using new for allocation, rewrite classes
...
// #define mempool_alloc the_thread_mempools.alloc
// #define mempool_free the_thread_mempools.free
// #define mempool_alloc_node the_thread_mempools.alloc
// #define mempool_free_node the_thread_mempools.free

// #define nvmpool_alloc the_thread_nvmpools.alloc
// #define nvmpool_free the_thread_nvmpools.free
// #define nvmpool_alloc_node the_thread_nvmpools.alloc
// #define nvmpool_free_node the_thread_nvmpools.free

#define mempool_alloc allocate
#define mempool_free free
#define mempool_alloc_node allocate
#define mempool_free_node free

#define nvmpool_alloc allocate
#define nvmpool_free free
#define nvmpool_alloc_node allocate
#define nvmpool_free_node free
